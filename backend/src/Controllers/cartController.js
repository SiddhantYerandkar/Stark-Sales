...
exports.addToCart = async (req, res) => {
    try {
        ...
        if (!cart) {
            ...
            if (!cartId || cartId.length == 0 || !isValidObjectId(cartId)) {
                return res.status(400).send({ status: false, message: "Cart Id is not present or is not a valid objectId" })
            }
...
exports.fetchCart = async (req, res) => {
    try {
        ...
        let getCartData = await cartModel.findOne({ userId: userId }).populate({
...
exports.updateCart = async (req, res) => {
    try {
        ...
        let result = await cartModel.findOneAndUpdate({ _id: cartId }, { $set: updateData }, { new: true }).populate({
            path: 'items.productId',
            select: 'title price productImage'
